...
{
    public class DetentionLetterProcessor
    {
        private readonly IConfiguration _configuration;

        public DetentionLetterProcessor(IConfiguration configuration)
        {
        ...
        private bool SendEmail(string SentFrom, string SentTo, string SentCC, string Subject, string SentBody, List<Attachment> SentAttachments)
        {
            ...
            try
            {
                ...
                NetworkCredential basicCredential1 = GetDecryptedCredential(username, password);
        ...
        private byte[] DownLoadFileByWebRequest(Guid pOrderId, string reportName, Guid pUserId)
        {
            try
            {
                string crmUrl = _configuration["CRMReportExecution:Url"];
