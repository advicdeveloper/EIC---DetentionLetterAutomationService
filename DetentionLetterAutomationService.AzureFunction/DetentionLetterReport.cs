...
namespace DetentionLetterAutomationService.AzureFunction
{
    public class DetentionLetterReport
    {
        ...
        public DetentionLetterReport(ILogger logger)
        {
            _logger = logger;
        }
        ...
        public void GenerateReport(BE.OrderSummary dlOrderSummary)
        {
            ...
            {
                _logger.LogError(ex, "Exception in GenerateReport function");
            }
        }
        ...
        private bool UploadReport(byte[] filedata, string dirPAth, string fPath)
        {
            ...
            {
                _logger.LogError(ex, "Exception in UploadReport function");
            }
            return false;
        }
        ...
        public bool SendReportEmail(BE.OrderSummary summary, List<BE.Users> lstUserCC)
        {
            ...
            try
            {
                string emailFrom = Environment.GetEnvironmentVariable("EmailFrom") ?? string.Empty;
                ...
                foreach (BE.OrderHistory hrec in lstHistory)
                {
                    ...
                    {
                        Attachment messagetransmittalAttachment = new Attachment(Path.Combine(appPath, "LetterDocuments", "NCSPA Installation Manual for CSP, Pipe Arches and Structural Plate.pdf"));
                        emailAttachment.Add(messagetransmittalAttachment);
                    }
                    ...
                    {
                        Attachment messagetransmittalAttachment = new Attachment(Path.Combine(appPath, "LetterDocuments", "CMP Detention Installation Guide.pdf"));
                        emailAttachment.Add(messagetransmittalAttachment);
                    }
                    ...
                    {
                        Attachment messagetransmittalAttachment = new Attachment(Path.Combine(appPath, "LetterDocuments", "DuroMaxx SRPE-Tank Installation Guide.pdf"));
                        emailAttachment.Add(messagetransmittalAttachment);
                    }
                    ...
                    {
                        Attachment messagetransmittalAttachment = new Attachment(Path.Combine(appPath, "LetterDocuments", "DMX Installation Guide.pdf"));
                        emailAttachment.Add(messagetransmittalAttachment);
                    }
                    ...
                    {
                        Attachment messagetransmittalAttachment = new Attachment(Path.Combine(appPath, "LetterDocuments", "DMX Detention Installation Guide.pdf"));
                        emailAttachment.Add(messagetransmittalAttachment);

                        Attachment messagetransmittalAttachment1 = new Attachment(Path.Combine(appPath, "LetterDocuments", "CMP Detention Installation Guide.pdf"));
        ...
        public void SendMissingDetailEmail(Guid modifiedby, string ordernumber)
        {
            try
            {
                ...
                {
                    _logger.LogError("SendMissingDetailEmail->SendEmail: No Mail Recipient/Sender - Subject {fullName}", ordermodiedby.FullName);
                    return;
                }
                ...
                _logger.LogInformation("{emailSubject} sent mail to Modified USER[SendMissingDetailEmail]-{emailTo}", emailSubject, emailTo);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Exception in SendMissingDetailEmail->SendEmail Subject {ordernumber}", ordernumber);
                throw ex;
            }
        }
        ...
        private bool SendEmail(string SentFrom, string SentTo, string SentCC, string Subject, string SentBody, List<Attachment> SentAttachments)
        {
            ...
            SmtpClient mailClient = new SmtpClient(
                Environment.GetEnvironmentVariable("EmailHost") ?? string.Empty,
                Convert.ToInt32(Environment.GetEnvironmentVariable("EmailHostPort") ?? "25"));
            ...
            try
            {
                ...
                NetworkCredential basicCredential1 = GetDecryptedCredential(
                    Environment.GetEnvironmentVariable("EmailSender") ?? string.Empty,
                    Environment.GetEnvironmentVariable("EmailSenderPwd") ?? string.Empty);
        ...
        public void UpdateStatus(OrderSummary summary, bool pSucessfull, string message)
        {
            ...
            {
                _logger.LogError(ex, "Exception in UpdateStatus function");
            }
        }
        ...
        public bool InsertOrderHistory(BE.OrderSummary _ordersummary, string reportName)
        {
            ...
            {
                _logger.LogError(ex, "Exception in InsertOrderHistory function");
            }
            return false;
        }
        ...
        private byte[] DownLoadFileByWebRequest(Guid pOrderId, string reportName, Guid pUserId)
        {
            try
            {
                string crmUrl = Environment.GetEnvironmentVariable("ReportWebURL") ?? string.Empty;
            ...
            {
                _logger.LogError(ex, "Exception in DownLoadFileByWebRequest ('{pOrderId}') and  ('{reportName}') function", pOrderId, reportName);
                return null;
            }
        }
